...
def basic_shares_outstanding(x: str) -> str:
    """Returns the basic share outstanding."""
    try:
        values = x[1:-1].split(", ")
        seriesA,seriesB,seriesC=map(int,values)
        basic_shares_outstanding_result= seriesA+seriesB+seriesC
        print("SP",basic_shares_outstanding_result)
        return basic_shares_outstanding_result
    except:
        em="Give me the series A , series B and series C for the said company"
        return em
